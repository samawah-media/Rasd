...
- RSS polling now skips entries that do not match the active Hidayathon keyword rule and reports them as skipped/non-matching rather than inserting unrelated review items.
- `/api/keyword-rules` lists and updates the active Hidayathon keyword dictionary. `/ops` exposes this to owner/editor as primary signals, context words, and excluded terms.
